"""Local diagnostic application: no model calls or external actions."""
import logging
import os
import sqlite3
...
<small>This verifies local request processing and storage. AI agents, research, email and self-improvement are not enabled. Submitted text is stored on this computer.</small>
<form id="form"><p><label for="text">Describe what you want to check</label></p>
<textarea id="text" maxlength="4000" required></textarea><button id="run">Run local diagnostic</button></form>
...
document.getElementById('form').addEventListener('submit',async e=>{
 ...
 button.disabled=true; output.textContent='Running local diagnostic…';
 try {
  const data=await call('/api/requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'diagnose',text:document.getElementById('text').value})});
